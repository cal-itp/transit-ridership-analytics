...
def ingest_omnitrans(
    ...
    raw_omni_export = (
        raw_omni.groupby(["FiscalYear", "Route", "Stop Name"], dropna=False)[["Total Board", "Total Alight"]]
        .sum()
        .reset_index()
    )
    ...
    raw_omni_export["fiscal_year_days"] = raw_omni_export["FiscalYear"].apply(
...
if __name__ == "__main__":
    ...
    raw_omni_export.to_parquet(f"{RAW_GCS}{agency_name}/ridership_round1.parquet", filesystem=gcsfs.GCSFileSystem())
