...
def export_excel_as_parquet(
    ...
    for filename in list_of_files:
        df = pd.read_excel(f"{LOCAL_FOLDER}{agency_name}/{filename}", engine="openpyxl")
        ...
        df.to_parquet(f"{AGENCY_GCS}{agency_name}/{Path(filename).stem}.parquet", filesystem=gcsfs.GCSFileSystem())
        print(f"exported {filename} as parquet")

    return


def ingest_samtrans(agency_name: str = "samtrans") -> pd.DataFrame:
    """
    ...
    raw_samtrans_agg = (
        raw_samtrans.groupby(["Route", "APC Date", "Stop ID", "Stop Name"], dropna=False)
    ...
    stop_loc_map = (
        raw_samtrans.groupby(["Stop ID", "Stop Name"], dropna=False).agg({"Lat": "max", "Lon": "max"}).reset_index()
    )

    raw_samtrans_export = pd.merge(raw_samtrans_agg, stop_loc_map, on=["Stop ID", "Stop Name"], how="left")
...
if __name__ == "__main__":
    ...
    raw_samtrans_export.to_parquet(f"{RAW_GCS}{agency_name}/ridership_round1.parquet", filesystem=gcsfs.GCSFileSystem())
