...
def export_as_csv(gdf: gpd.GeoDataFrame, export_filename: str):
    """
    Export as csv.
    ...
    gdf = (
        gdf.assign(stop_lon=gdf.geometry.x, stop_lat=gdf.geometry.y)
    ...
    gdf.to_csv(f"{gcs_parent_path}{name}.csv", index=False)
...
def export_as_geojson(
    ...
    gdf = gdf.astype({c: "str" for c in date_cols})

    gcs_parent_path, name = split_filename_into_folder_and_name(export_filename)

    utils.geojson_gcs_export(gdf, f"{gcs_parent_path}", name, geojson_type="geojson")
...
if __name__ == "__main__":
    ...
    gdf = gpd.read_parquet(
        f"{INTERMED_GCS}dim_stops_with_feed_service_period.parquet", storage_options={"token": credentials.token}
    )
    ...
    for f in filepaths:
        publish_utils.write_to_public_gcs(f, f"transit_ridership/{Path(f).name}", PUBLIC_GCS)
