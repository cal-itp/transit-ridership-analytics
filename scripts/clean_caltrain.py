...
def ingest_caltrain(agency_name: str = "caltrain") -> pd.DataFrame:
    """
    ...
    raw_caltrain = pd.read_excel(f"{LOCAL_FOLDER}{agency_name}/{filename}", engine="openpyxl").reset_index(drop=True)

    raw_caltrain["Month, Year of Date"] = raw_caltrain["Month, Year of Date"].ffill()
    raw_caltrain = pd.melt(
        raw_caltrain, id_vars=["Month, Year of Date", "Origin Station", "Caltrain Ridership"]
    ).rename(columns={"variable": "Date Type", "value": "Average Ridership", "Month, Year of Date": "Month"})
...
if __name__ == "__main__":
    ...
    raw_caltrain.to_parquet(f"{RAW_GCS}{agency_name}/ridership_round1.parquet", filesystem=gcsfs.GCSFileSystem())
