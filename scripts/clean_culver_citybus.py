...
def ingest_culver_citybus(agency_name: str = "culver_citybus") -> pd.DataFrame:
    """
    Import Culver City.
    ...
    raw_culver_city = pd.read_csv(f"{LOCAL_FOLDER}{agency_name}/{filename}", skiprows=[1])

    raw_culver_city_export = (
        raw_culver_city.groupby(["Day Of Week", "Route", "Direction", "Stop ID", "Stop Name"], dropna=False)[
            ["Time Period AVG On", "Time Period AVG Off"]
        ]
        .sum()
        .reset_index()
        .rename(columns={"Time Period AVG On": "AVG On", "Time Period AVG Off": "AVG Off"})
    )
