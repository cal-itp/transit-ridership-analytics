...
def ingest_golden_gate_transit(
    ...
    raw_ggt = pd.read_csv(f"{LOCAL_FOLDER}{agency_name}/{filename}", encoding="utf-8")
    # filter out virtual time points
    t_df_ggt = raw_ggt[raw_ggt["POINT_ROLE"].isin(["ST", "S"])]

    raw_ggt_export = (
        t_df_ggt.groupby(by=["OPERATION_DATE", "ROUTE", "DIRECTION", "STOP_NUMBER", "STOP_NAME"], dropna=False)[
...
if __name__ == "__main__":
    ...
    raw_ggt_export.to_parquet(f"{RAW_GCS}{agency_name}/ridership_round1.parquet", filesystem=gcsfs.GCSFileSystem())
