...
def process_individual_sheet(filename: str, sheet_name: str) -> pd.DataFrame:
    """
    ...
    t_df_sbmtd_long["start_date"] = t_df_sbmtd_long["month_str"].apply(
        lambda x: x.replace(day=1) if pd.notnull(x) else None
    )
    t_df_sbmtd_long["end_date"] = t_df_sbmtd_long["month_str"].apply(
        lambda x: x.replace(day=1) + pd.tseries.offsets.MonthEnd(1) if pd.notnull(x) else None
    )
...
def ingest_sbmtd(agency_name: str = "sbmtd") -> pd.DataFrame:
    """
    ...
    t_dfs = [
        process_individual_sheet(f"{LOCAL_FOLDER}{agency_name}/{filename}", one_sheet_name)
        for one_sheet_name in sheet_names
    ]
...
if __name__ == "__main__":
    ...
    raw_sbmtd.to_parquet(f"{RAW_GCS}{agency_name}/ridership_round1.parquet", filesystem=gcsfs.GCSFileSystem())
