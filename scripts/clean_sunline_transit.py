...
def make_sunline_df_long(
    raw_sunline_df: pd.DataFrame, ridership_meta_df: pd.DataFrame, static_cols: list
) -> pd.DataFrame:
...
def ingest_sunline_transit(
    ...
    raw_sunline.columns = [
        static_cols[i] if i < len(static_cols) else raw_sunline.columns[i] for i in range(len(raw_sunline.columns))
    ]
    ...
    for c in ["APC Alights", "APC Boards", "Avg. Alights", "Avg. Boards"]:
        raw_sunline_export[c] = pd.to_numeric(
            raw_sunline_export.apply(lambda x: str(x[c]).strip().replace("-", "").replace("nan", ""), axis=1)
        )
...
if __name__ == "__main__":
    ...
    raw_sunline_export.to_parquet(f"{RAW_GCS}{agency_name}/ridership_round1.parquet", filesystem=gcsfs.GCSFileSystem())
