...
def clean_individual_excel(filename: str):
    ...
    """
    raw_scm = pd.read_excel(f"{LOCAL_FOLDER}{agency_name}/{filename}", header=[0, 1], engine="openpyxl")
    raw_scm.columns = flatten_columns_scm(raw_scm.columns)

    raw_scm = raw_scm[
        ~raw_scm["Stop Name"].str.startswith(("Total", "Minimum", "Maximum", "Average", "Std. Dev."), na=False)
    ]
    ...
    raw_scm_grouped = (
        raw_scm.groupby(by=["Stop Name", "Stop ID", "filename"], as_index=False, dropna=False)
...
def ingest_santa_cruz_metro(
    ...
    raw_scm_export = (
        raw_scm_export.groupby(by=["Stop Name", "Stop ID", "start_date", "end_date", "schedule_name"], dropna=False)
        .agg({"Boardings": "sum", "Alightings": "sum"})
        .reset_index()
    )
...
if __name__ == "__main__":
    ...
    raw_scm_export.to_parquet(f"{RAW_GCS}{agency_name}/ridership_round1.parquet", filesystem=gcsfs.GCSFileSystem())
