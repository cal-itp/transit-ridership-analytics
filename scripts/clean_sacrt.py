...
def ingest_sacrt_ridership(agency_name: str = "sacrt", filename: str = "ridership.txt.csv") -> pd.DataFrame:
    """
    ...
    raw_sacrt_ridership_grouped = (
        raw_sacrt_ridership.groupby(
            [
            ...
            dropna=False,
        )[["average_boardings", "average_alightings"]]
        .sum()
        .reset_index()
    )

    raw_sacrt_ridership_grouped = raw_sacrt_ridership_grouped.assign(
        start_date=pd.to_datetime(raw_sacrt_ridership_grouped["ridership_start_date"], format="%Y%m%d").dt.strftime(
            "%Y-%m-%d"
        ),
        end_date=pd.to_datetime(raw_sacrt_ridership_grouped["ridership_end_date"], format="%Y%m%d").dt.strftime(
            "%Y-%m-%d"
        ),
    )
...
def ingest_sacrt_stops(agency_name: str = "sacrt", filename: str = "stops.txt.csv") -> pd.DataFrame:
    """
...
def ingest_sacrt_routes(agency_name: str = "sacrt", filename: str = "routes.txt.csv") -> pd.DataFrame:
    """
...
def merge_ridership_with_stop_and_routes(
    ridership_df: pd.DataFrame, stop_df: pd.DataFrame, routes_df: pd.DataFrame
) -> pd.DataFrame:
    """
    ...
    """
    raw_sacrt_export = pd.merge(ridership_df, stop_df, how="left", on="stop_id").merge(
        routes_df, how="left", on="route_id"
    )
    ...
    raw_sacrt_export = raw_sacrt_export.assign(
        ...
        day_type=raw_sacrt_export.apply(
            lambda x: "weekday" if x.saturday == 0 and x.sunday == 0 else "all",
            axis=1,
        ),
...
if __name__ == "__main__":
    ...
    raw_sacrt_export.to_parquet(f"{RAW_GCS}{agency_name}/ridership_round1.parquet", filesystem=gcsfs.GCSFileSystem())
