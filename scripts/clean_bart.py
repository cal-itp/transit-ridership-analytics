...
def ingest_bart_entries_and_exits(agency_name: str = "bart", sheet_name: str = "Daily Raw Data") -> pd.DataFrame:
    """
    Import BART entries and exit Excel and clean up columns.
    """
    filename = RAW_DATA_YAML[agency_name][0]
    raw_bart = pd.read_excel(
        f"{LOCAL_FOLDER}{agency_name}/{filename}", sheet_name=sheet_name, header=[0, 1], engine="openpyxl"
    )
    ...
    ridership_cols = [
        c for c, top in zip(raw_bart.columns, top_headers) if ("Exits" in str(top)) or ("Entries" in str(top))
    ]

    # melt ridership columns
    t_df_ridership = raw_bart.melt(id_vars=meta_cols, value_vars=ridership_cols, value_name="ridership").rename(
        columns={"variable_0": "type", "variable_1": "Station"}
    )

    # pivot table
    t_df_ridership_pivot = t_df_ridership.pivot_table(
        index=meta_cols + ["Station"], columns="type", values="ridership", aggfunc="sum"
    ).reset_index()

    t_df_ridership_pivot.columns = [
        bottom_headers[i] if i < len(meta_cols) else t_df_ridership_pivot.columns[i]
...
def ingest_bart_station_crosswalk(agency_name: str = "bart", sheet_name: str = "Station Crosswalk") -> pd.DataFrame:
    """
    ...
    raw_bart_station = pd.read_excel(
        f"{LOCAL_FOLDER}{agency_name}/{filename}", sheet_name=sheet_name, engine="openpyxl"
    )
...
def merge_bart_entries_exits_station(
    ...
    # add station name
    raw_bart_export = (
        pd.merge(entries_df, station_df, how="left", left_on="Station", right_on="Station Code")
        .drop(columns="Station Code")
        .astype({"Station": "str"})
    )
...
if __name__ == "__main__":
    ...
    raw_bart_export.to_parquet(f"{RAW_GCS}{agency_name}/ridership_round1.parquet", filesystem=gcsfs.GCSFileSystem())
